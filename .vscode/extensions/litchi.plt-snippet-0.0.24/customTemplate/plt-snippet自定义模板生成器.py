""" 读取 toml 文件中配置的模板选项，生成可供 VS Code 的 plt-snippet 插件加载的网页
"""
import toml
import argparse
import base64
...
def checkDict(dt: dict):
    """
    检查字典中是否包含所有指定的键，若缺少任何一个则抛出 KeyError。

    :param dict: 需要检查的字典
    """
    required_keys = ["name", "id", "png", "ipynb"]
    missing_keys = [key for key in required_keys if key not in dt]
    if missing_keys:
        print(f"toml文件中条目 {dt} 缺乏以下项目: {', '.join(missing_keys)}")
        exit(-1)
    return 0
...
class TemplateElement:
    ...
    def __init__(self, name, id, png, ipynb):
        self.name = name 
        self.id = id
        self.png = png 
        self.ipynb = ipynb # read_file_content(ipynb) 
    
    @classmethod
    def fromDict(cls, dict):
        checkDict(dict)
        return cls(dict["name"], dict["id"], dict["png"], dict["ipynb"])
    
    
    def __repr__(self):
        return f"""<div class="box">
<img src="{self.png}" alt="{self.name}">
<button id="{self.id}" data-ipynbpath="{self.ipynb}">{self.name}</button>
</div>\n"""
    

def toml2html(filename:str):
    body = """<body>
<div class="container">\n"""
    ...
    # 访问解析后的数据
    for _, group_data in parsed_toml.items():
        body += f"<h2>{group_data['title']}</h2>\n"
        
        for item in group_data['item']:
            te = TemplateElement.fromDict(item)
            body += str(te)
...
def png_base64(htmlstr:str) -> str:
    # 使用 BeautifulSoup 解析 HTML
    soup = BeautifulSoup(htmlstr, 'html.parser')

    # 找到所有的图片标签并循环处理每个图片标签为 base64 编码
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="读取 toml 格式文件，输出 VS Code 的 plt-snippet 插件可加载网页。")
    
    # 添加参数
    parser.add_argument('-f', '--filename', default='template.toml', help='toml 格式文件')
    parser.add_argument('-o', '--output', default='template.html', help='输出网页文件名')

    # 解析参数
    args = parser.parse_args()
    
    # 获取html
    html = toml2html(args.filename)
    # html = png_base64(html)
    
    with open(args.output, 'w', encoding='utf-8') as f:
        f.write(html)
    print(f"\nhtml 文件 {args.output} 写出成功")
...
#  打包命令：nuitka.bat .\plt-snippet自定义模板生成器.py